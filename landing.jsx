...
const Benefit = ({text, color}) => (
  <div style={{display:"flex", alignItems:"flex-start", gap:"0.75rem"}}>
    <div style={{width:20, height:20, borderRadius:"50%", background:`${color}20`, border:`1px solid ${color}60`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.65rem", color, fontWeight:800, flexShrink:0, marginTop:2}}>✓</div>
    <span style={{fontSize:"0.9rem", color:T.text, lineHeight:1.55}}>{text}</span>
  </div>
);
...
const DashboardMockup = () => {
  const cards = [
    {label:"Open",value:"8",sub:"6 high priority",icon:"🔴"},
    {label:"In Progress",value:"3",sub:"18 total",icon:"🟡"},
    {label:"Resolved",value:"7",sub:"Closed tickets",icon:"🟢"},
    {label:"Escalations",value:"1",sub:"2 total",icon:"⚡"},
    {label:"Pending Refunds",value:"4",sub:"€578 pending",icon:"💳"},
    {label:"Open Disputes",value:"5",sub:"View all →",icon:"⚖️"},
  ];
  return (
    <div style={{background:"#0F1117", borderRadius:20, overflow:"hidden", boxShadow:"0 24px 80px rgba(0,0,0,0.4)", border:"1px solid rgba(255,255,255,0.06)"}}>
      {/* Window chrome */}
      <div style={{padding:"0.75rem 1rem", background:"#1a1d27", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", alignItems:"center", gap:"0.5rem"}}>
        {["#ef4444","#f59e0b","#22c55e"].map(c=><div key={c} style={{width:10,height:10,borderRadius:"50%",background:c}}/>)}
        <span style={{marginLeft:"0.5rem", fontSize:"0.75rem", color:"rgba(255,255,255,0.3)", fontWeight:500}}>dashboard.jm20.eu</span>
      </div>
      <div style={{display:"flex", minHeight:340}}>
        {/* Sidebar */}
        <div style={{width:160, background:"#0a0d14", padding:"1.25rem 0", borderRight:"1px solid rgba(255,255,255,0.05)", flexShrink:0}}>
          <div style={{padding:"0 1rem 1.25rem", borderBottom:"1px solid rgba(255,255,255,0.06)", marginBottom:"0.75rem"}}>
            <div style={{display:"flex", alignItems:"center", gap:"0.4rem"}}>
              <div style={{display:"flex", gap:2}}>{["#C4633A","#7c6af7","#22c55e"].map(c=><div key={c} style={{width:6,height:6,borderRadius:"50%",background:c}}/>)}</div>
              <span style={{fontWeight:800, fontSize:"0.8rem", color:"#fff"}}>jm20</span>
            </div>
            <div style={{fontSize:"0.65rem", color:T.terra, fontWeight:700, marginTop:"0.3rem", letterSpacing:"0.06em"}}>AI Solutions</div>
          </div>
          {[
            ...
            <div key={i} style={{padding:"0.5rem 1rem", display:"flex", alignItems:"center", gap:"0.5rem", background:item.active?"rgba(196,99,58,0.15)":"transparent", borderLeft:item.active?`2px solid ${T.terra}`:"2px solid transparent", cursor:"pointer"}}>
              <span style={{fontSize:"0.75rem", color:item.active?"#fff":"rgba(255,255,255,0.35)"}}>{item.icon}</span>
              <span style={{fontSize:"0.75rem", color:item.active?"#fff":"rgba(255,255,255,0.35)", fontWeight:item.active?600:400}}>{item.label}</span>
            </div>
          ))}
        </div>
        {/* Main */}
        <div style={{flex:1, padding:"1.25rem", overflow:"hidden"}}>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"1rem"}}>
            <div>
              <div style={{fontSize:"1.1rem", fontWeight:800, color:"#fff"}}>Dashboard</div>
              <div style={{fontSize:"0.72rem", color:"rgba(255,255,255,0.4)", marginTop:2}}>Customer support overview</div>
            </div>
            <div style={{display:"flex", gap:"0.25rem"}}>
              {["24h","7d","30d","All"].map((p,i)=>(
                <div key={p} style={{padding:"0.2rem 0.5rem", borderRadius:6, background:i===3?"#C4633A":"rgba(255,255,255,0.06)", fontSize:"0.7rem", color:i===3?"#fff":"rgba(255,255,255,0.4)", fontWeight:600, cursor:"pointer"}}>{p}</div>
              ))}
            </div>
          </div>
          <div style={{fontSize:"0.65rem", color:"rgba(255,255,255,0.3)", letterSpacing:"0.08em", fontWeight:600, marginBottom:"0.5rem"}}>TICKETS</div>
          <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"0.5rem", marginBottom:"0.75rem"}}>
            {cards.slice(0,3).map((c,i)=>(
              <div key={i} style={{background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:"0.75rem"}}>
                <div style={{fontSize:"0.7rem", color:"rgba(255,255,255,0.45)", marginBottom:"0.25rem"}}>{c.label}</div>
                <div style={{fontSize:"1.5rem", fontWeight:800, color:"#fff", lineHeight:1}}>{c.value}</div>
                <div style={{fontSize:"0.65rem", color:"rgba(255,255,255,0.3)", marginTop:"0.25rem"}}>{c.sub}</div>
              </div>
            ))}
          </div>
          <div style={{fontSize:"0.65rem", color:"rgba(255,255,255,0.3)", letterSpacing:"0.08em", fontWeight:600, marginBottom:"0.5rem"}}>OTHER</div>
          <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"0.5rem", marginBottom:"0.75rem"}}>
          ...
          <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.5rem"}}>
            <div style={{background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:"0.75rem"}}>
              <div style={{fontSize:"0.7rem", color:"rgba(255,255,255,0.45)", marginBottom:"0.25rem"}}>AI Interactions</div>
              <div style={{fontSize:"1.5rem", fontWeight:800, color:"#fff"}}>229</div>
              <div style={{fontSize:"0.65rem", color:"rgba(255,255,255,0.3)"}}>All time</div>
            </div>
            <div style={{background:`linear-gradient(135deg, ${T.terra}22, ${T.terra}08)`, border:`1px solid ${T.terra}40`, borderRadius:10, padding:"0.75rem"}}>
              <div style={{fontSize:"0.7rem", color:T.terraLight, marginBottom:"0.25rem"}}>AI Savings</div>
              <div style={{fontSize:"1.5rem", fontWeight:800, color:"#fff"}}>$1,940</div>
              <div style={{fontSize:"0.65rem", color:T.terraLight, opacity:.7}}>Estimated saved</div>
            </div>
          </div>
        </div>
      </div>
...
/* ── NL2SQL MOCKUP ── */
const NL2SQLMockup = () => (
  <div style={{background:"#0F1117", borderRadius:20, overflow:"hidden", boxShadow:"0 24px 80px rgba(0,0,0,0.35)", border:"1px solid rgba(255,255,255,0.06)"}}>
    <div style={{padding:"0.75rem 1rem", background:"#1a1d27", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", alignItems:"center", gap:"0.5rem"}}>
      {["#ef4444","#f59e0b","#22c55e"].map(c=><div key={c} style={{width:10,height:10,borderRadius:"50%",background:c}}/>)}
      <span style={{marginLeft:"0.5rem", fontSize:"0.75rem", color:"rgba(255,255,255,0.3)"}}>NL2SQL — Ask your data anything</span>
    </div>
    <div style={{padding:"1.25rem"}}>
      <div style={{background:"rgba(124,106,247,0.1)", border:"1px solid rgba(124,106,247,0.3)", borderRadius:10, padding:"0.875rem 1rem", marginBottom:"1rem", display:"flex", alignItems:"center", gap:"0.75rem"}}>
        <span style={{fontSize:"0.9rem", flex:1, color:"rgba(255,255,255,0.8)"}}>Show me total revenue for the last 30 days</span>
        <div style={{background:"#7c6af7", borderRadius:6, padding:"0.3rem 0.75rem", fontSize:"0.75rem", color:"#fff", fontWeight:600, flexShrink:0}}>Run →</div>
      </div>
      <div style={{background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:"1rem", marginBottom:"1rem", fontFamily:"monospace", fontSize:"0.78rem", lineHeight:1.7}}>
        <span style={{color:"#7c6af7"}}>SELECT</span> <span style={{color:"rgba(255,255,255,0.7)"}}>DATE_TRUNC</span><span style={{color:"rgba(255,255,255,0.4)"}}>('day',</span> <span style={{color:"#f59e0b"}}>created_at</span><span style={{color:"rgba(255,255,255,0.4)"}}>)</span> <span style={{color:"rgba(255,255,255,0.4)"}}>AS</span> <span style={{color:"#22c55e"}}>day</span>,<br/>
        &nbsp;&nbsp;&nbsp;&nbsp;<span style={{color:"rgba(255,255,255,0.7)"}}>SUM</span><span style={{color:"rgba(255,255,255,0.4)"}}>(<span style={{color:"#f59e0b"}}>total</span>)</span> <span style={{color:"rgba(255,255,255,0.4)"}}>AS</span> <span style={{color:"#22c55e"}}>revenue</span><br/>
        <span style={{color:"#7c6af7"}}>FROM</span> <span style={{color:"#f59e0b"}}>invoices</span><br/>
        <span style={{color:"#7c6af7"}}>WHERE</span> <span style={{color:"#f59e0b"}}>created_at</span> <span style={{color:"rgba(255,255,255,0.4)"}}>&gt;= NOW() - INTERVAL</span> <span style={{color:"#22c55e"}}>'30 days'</span><br/>
        &nbsp;&nbsp;&nbsp;&nbsp;<span style={{color:"rgba(255,255,255,0.4)"}}>AND</span> <span style={{color:"#f59e0b"}}>status</span> = <span style={{color:"#22c55e"}}>'paid'</span><br/>
        <span style={{color:"#7c6af7"}}>GROUP BY</span> 1 <span style={{color:"#7c6af7"}}>ORDER BY</span> 1
      </div>
      <div style={{background:"rgba(124,106,247,0.08)", border:"1px solid rgba(124,106,247,0.2)", borderRadius:8, padding:"0.625rem 0.875rem", fontSize:"0.8rem", color:"rgba(255,255,255,0.5)", marginBottom:"1rem"}}>
        💡 Returns daily revenue totals for paid invoices over the last 30 days
      </div>
      <div style={{border:"1px solid rgba(255,255,255,0.07)", borderRadius:8, overflow:"hidden"}}>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", background:"rgba(255,255,255,0.05)", padding:"0.5rem 0.75rem"}}>
          {["day","revenue"].map(h=><div key={h} style={{fontSize:"0.72rem", fontWeight:700, color:"rgba(255,255,255,0.5)", letterSpacing:"0.04em"}}>{h}</div>)}
        </div>
        {[["2024-01-28","€3,241.00"],["2024-01-29","€2,890.50"],["2024-01-30","€4,102.75"],["2024-01-31","€3,677.20"]].map((row,i)=>(
          <div key={i} style={{display:"grid", gridTemplateColumns:"1fr 1fr", padding:"0.5rem 0.75rem", background:i%2===0?"rgba(255,255,255,0.02)":"transparent", borderTop:"1px solid rgba(255,255,255,0.04)"}}>
            {row.map((cell,ci)=><div key={ci} style={{fontSize:"0.78rem", color:ci===1?"#22c55e":"rgba(255,255,255,0.55)"}}>{cell}</div>)}
          </div>
        ))}
      </div>
    </div>
...
const AgentMockup = () => {
  ...
  const msgs = {
    whatsapp:[
      {from:"user",text:"Hello, I was charged twice for my subscription"},
      {from:"bot",text:"Hi! Let me check that immediately 🔍 Could you confirm your email?"},
      {from:"user",text:"maria@example.com"},
      {from:"bot",text:"Found it, María! Duplicate charge of €29.99 confirmed. Refund initiated — 2–3 business days. Flagged with billing team to prevent recurrence ✅"},
    ],
    telegram:[
      {from:"user",text:"/start"},
      {from:"bot",text:"👋 Welcome to Acme Support on Telegram! How can I help?"},
      {from:"user",text:"I need to return a product"},
      {from:"bot",text:"Sure! Share your order number (starts with #ORD-) and I'll get that started for you."},
    ],
    web:[
      {from:"user",text:"Where is my order? It's been 3 days"},
      {from:"bot",text:"Hi! Order #ORD-8821 shipped yesterday via DHL, arriving tomorrow before 6pm 📦 I'll send the tracking link now."},
      {from:"user",text:"Can I change the address?"},
      {from:"bot",text:"Since it's in transit, redirect directly with DHL using your tracking number. Want the link?"},
    ]
  };
  const cfgMap = {whatsapp:{color:"#25d366",label:"WhatsApp",Logo:WhatsAppLogo},telegram:{color:"#0088cc",label:"Telegram",Logo:TelegramLogo},web:{color:"#3b82f6",label:"Web Chat",Logo:WebChatLogo}};
  const cfg = cfgMap[ch];
  return (
    <div style={{background:"#0F1117", borderRadius:20, overflow:"hidden", boxShadow:"0 24px 80px rgba(0,0,0,0.35)", border:"1px solid rgba(255,255,255,0.06)"}}>
      <div style={{padding:"0.75rem 1rem", background:"#1a1d27", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", alignItems:"center", gap:"0.5rem"}}>
        {["#ef4444","#f59e0b","#22c55e"].map(c=><div key={c} style={{width:10,height:10,borderRadius:"50%",background:c}}/>)}
        <div style={{marginLeft:"0.5rem", display:"flex", gap:"0.375rem"}}>
          {Object.entries(cfgMap).map(([id,c])=>(
            <button key={id} onClick={()=>setCh(id)} style={{padding:"0.2rem 0.6rem", borderRadius:100, border:"none", background:ch===id?c.color:"rgba(255,255,255,0.07)", color:ch===id?"#fff":"rgba(255,255,255,0.4)", fontSize:"0.7rem", fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", gap:"0.25rem", transition:"all .15s"}}>
              <c.Logo size={11}/> {c.label}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{height:260, overflowY:"auto", padding:"1rem", display:"flex", flexDirection:"column", gap:"0.625rem"}}>
        {msgs[ch].map((m,i)=>(
          <div key={`${ch}-${i}`} style={{display:"flex", justifyContent:m.from==="user"?"flex-end":"flex-start", animation:"slideIn .25s ease both"}}>
            <div style={{maxWidth:"78%", padding:"0.6rem 0.875rem", borderRadius:m.from==="user"?"12px 12px 3px 12px":"12px 12px 12px 3px", background:m.from==="user"?cfg.color:"rgba(255,255,255,0.07)", color:"#fff", fontSize:"0.82rem", lineHeight:1.5}}>{m.text}</div>
          </div>
        ))}
      </div>
      <div style={{padding:"0.75rem 1rem", borderTop:"1px solid rgba(255,255,255,0.06)", display:"flex", gap:"0.5rem"}}>
        <div style={{flex:1, background:"rgba(255,255,255,0.06)", borderRadius:8, padding:"0.5rem 0.875rem", fontSize:"0.8rem", color:"rgba(255,255,255,0.25)"}}>Type a message…</div>
        <div style={{background:cfg.color, borderRadius:8, padding:"0.5rem 0.875rem", fontSize:"0.8rem", color:"#fff", fontWeight:600}}>↑</div>
      </div>
    </div>
  );
};
...
/* ── RAG MOCKUP ── */
const RAGMockup = () => (
  <div style={{background:T.surface, borderRadius:20, overflow:"hidden", boxShadow:T.shadowHover, border:`1px solid ${T.border}`}}>
    ...
    <div style={{padding:"1rem", display:"flex", flexDirection:"column", gap:"0.75rem", minHeight:280}}>
      {[
        {from:"bot",text:"👋 Hi! I can answer questions about our products, shipping, returns and billing. What do you need?"},
        {from:"user",text:"What's your return policy?"},
        {from:"bot",text:"Our return policy is 30 days from delivery. Items must be unused and in original packaging. Returns are free with our prepaid label. Refunds processed within 3–5 business days ✅"},
        {from:"user",text:"Do you ship to Portugal?"},
        {from:"bot",text:"Yes! We ship to all EU countries including Portugal 🇵🇹 Standard delivery 3–5 days, express 1–2 days."},
      ].map((m,i)=>(
        <div key={i} style={{display:"flex", justifyContent:m.from==="user"?"flex-end":"flex-start"}}>
          <div style={{maxWidth:"80%", padding:"0.6rem 0.875rem", borderRadius:m.from==="user"?"12px 12px 3px 12px":"12px 12px 12px 3px", background:m.from==="user"?"#22c55e":T.bg, color:m.from==="user"?"#fff":T.text, fontSize:"0.82rem", lineHeight:1.5, border:m.from==="user"?"none":`1px solid ${T.border}`}}>{m.text}</div>
        </div>
      ))}
    </div>
    <div style={{padding:"0.75rem 1rem", borderTop:`1px solid ${T.border}`, display:"flex", gap:"0.5rem"}}>
      <div style={{flex:1, background:T.bg, border:`1px solid ${T.border}`, borderRadius:8, padding:"0.5rem 0.875rem", fontSize:"0.8rem", color:T.muted}}>Ask a question…</div>
      <div style={{background:"#22c55e", borderRadius:8, padding:"0.5rem 0.875rem", fontSize:"0.8rem", color:"#fff", fontWeight:600}}>↑</div>
    </div>
  </div>
);

/* ── SERVICES — alternating product sections ── */
const PRODUCTS = [
  {
...
const Services = () => (
  <div id="services">
    ...
    {PRODUCTS.map((p,i) => (
      <section key={p.id} style={{background:p.bg, padding:"6rem 3rem", borderTop:p.dark?"none":`1px solid ${T.border}`}}>
        <div style={{maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"5rem", alignItems:"center", direction:p.reverse?"rtl":"ltr"}}>
          {/* Text */}
          <div style={{direction:"ltr"}}>
            <div style={{display:"inline-flex", alignItems:"center", gap:"0.4rem", background:p.tagBg, borderRadius:100, padding:"0.3rem 0.875rem", marginBottom:"1.25rem"}}>
              <div style={{width:6, height:6, borderRadius:"50%", background:p.accent}}/>
              <span style={{fontSize:"0.72rem", color:p.accent, fontWeight:700, letterSpacing:"0.08em"}}>{p.tag}</span>
            </div>
            <h2 style={{fontSize:"clamp(1.6rem,3.5vw,2.4rem)", fontWeight:800, color:p.dark?"#fff":T.text, letterSpacing:"-0.03em", lineHeight:1.1, marginBottom:"1rem"}}>{p.headline}</h2>
            <p style={{color:p.dark?"rgba(255,255,255,0.55)":T.muted, fontSize:"0.975rem", lineHeight:1.75, marginBottom:"2rem"}}>{p.sub}</p>
            <div style={{display:"flex", flexDirection:"column", gap:"0.875rem", marginBottom:"2.5rem"}}>
              {p.benefits.map(b=>(
                <div key={b} style={{display:"flex", alignItems:"flex-start", gap:"0.75rem"}}>
                  <div style={{width:20, height:20, borderRadius:"50%", background:`${p.accent}25`, border:`1px solid ${p.accent}60`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.65rem", color:p.accent, fontWeight:800, flexShrink:0, marginTop:2}}>✓</div>
                  <span style={{fontSize:"0.9rem", color:p.dark?"rgba(255,255,255,0.7)":T.text, lineHeight:1.55}}>{b}</span>
                </div>
              ))}
            </div>
            <a href="#demo" style={{display:"inline-flex", alignItems:"center", gap:"0.5rem", background:p.accent, color:"#fff", padding:"0.75rem 1.5rem", borderRadius:10, fontSize:"0.9rem", fontWeight:700, boxShadow:`0 4px 20px ${p.accent}40`}}>
              Try it live →
            </a>
          </div>
          {/* Visual */}
...
const SOURCES = [
  {icon:"🐬", label:"MySQL", sub:"Existing websites & apps"},
  {icon:"🍃", label:"MongoDB", sub:"Document databases"},
  {icon:"🔥", label:"Firebase", sub:"Mobile & web apps"},
  {icon:"📋", label:"Airtable", sub:"Operations & CRM"},
  {icon:"💳", label:"Stripe", sub:"Billing & payments"},
  {icon:"📁", label:"Notion", sub:"Docs & wikis"},
  {icon:"☁️", label:"S3 / CSV", sub:"Files & exports"},
  {icon:"❄️", label:"Snowflake", sub:"Data warehouses"},
  {icon:"📊", label:"Salesforce", sub:"Enterprise CRM"},
  {icon:"🐘", label:"PostgreSQL", sub:"Any Postgres DB"},
];

const NoMigration = () => {
  const [active, setActive] = useState(null);
  return (
    <section style={{padding:"6rem 3rem", background:T.bg}}>
      <div style={{maxWidth:1080, margin:"0 auto"}}>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:"5rem", alignItems:"center"}}>
          {/* Left */}
          <div>
            <span style={{fontSize:"0.75rem", color:T.terra, fontWeight:700, letterSpacing:"0.1em"}}>ZERO MIGRATION</span>
            <h2 style={{fontSize:"clamp(1.75rem,3.5vw,2.5rem)", fontWeight:800, color:T.text, marginTop:"0.6rem", marginBottom:"1rem", letterSpacing:"-0.025em", lineHeight:1.15}}>
              Keep your databases.<br/><span style={{color:T.terra}}>All of them.</span>
            </h2>
            <p style={{color:T.muted, fontSize:"0.95rem", lineHeight:1.7, marginBottom:"1rem"}}>
              No migration. No disruption. No risk. We connect to your existing systems through secure wrappers — MySQL, Airtable, Stripe, MongoDB, whatever you use — and query them all from one secure hub.
            </p>
            <p style={{color:T.muted, fontSize:"0.95rem", lineHeight:1.7, marginBottom:"2rem"}}>
              Your customers get instant AI support on WhatsApp. Your team asks questions in plain English. You see everything in one dashboard. All without moving a single row of data.
            </p>
            <div style={{background:T.warm, border:`1px solid ${T.terraLight}`, borderRadius:12, padding:"1.25rem 1.5rem", marginBottom:"1.5rem"}}>
              <div style={{fontSize:"0.8rem", fontWeight:700, color:T.terra, marginBottom:"0.5rem", letterSpacing:"0.04em"}}>EXAMPLE QUERY ACROSS SYSTEMS</div>
              <div style={{fontFamily:"monospace", fontSize:"0.82rem", color:T.text, lineHeight:1.7, background:T.surface, borderRadius:8, padding:"0.875rem 1rem", border:`1px solid ${T.border}`}}>
                <span style={{color:"#7c6af7"}}>SELECT</span> s.customer_name,<br/>
                {"  "}<span style={{color:T.terra}}>SUM</span>(stripe.revenue) as revenue,<br/>
                {"  "}<span style={{color:"#2563eb"}}>COUNT</span>(tickets.id) as support_tickets<br/>
                <span style={{color:"#7c6af7"}}>FROM</span> stripe_customers s<br/>
                <span style={{color:"#7c6af7"}}>JOIN</span> support_tickets tickets <span style={{color:"#7c6af7"}}>ON</span> s.id = tickets.customer_id<br/>
                <span style={{color:"#7c6af7"}}>WHERE</span> tickets.created_at {">"} <span style={{color:"#16a34a"}}>'2024-01-01'</span><br/>
                <span style={{color:"#7c6af7"}}>GROUP BY</span> s.customer_name
              </div>
              <div style={{fontSize:"0.78rem", color:T.muted, marginTop:"0.625rem"}}>↑ Stripe + your support DB joined in one query — no export, no spreadsheet, no waiting.</div>
            </div>
            <div style={{display:"flex", flexDirection:"column", gap:"0.625rem"}}>
              {[
                "No data migration required",
                "Connect to 10+ database types",
                "Query across all sources in plain English",
                "All data stays in EU infrastructure",
              ].map(t => (
                <div key={t} style={{display:"flex", alignItems:"center", gap:"0.625rem"}}>
                  <div style={{width:18, height:18, borderRadius:"50%", background:T.terraLight, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.7rem", color:T.terra, fontWeight:800, flexShrink:0}}>✓</div>
                  <span style={{fontSize:"0.875rem", color:T.text, fontWeight:500}}>{t}</span>
          ...
          {/* Right — source grid */}
          <div>
            <div style={{fontSize:"0.78rem", fontWeight:700, color:T.muted, letterSpacing:"0.06em", marginBottom:"1rem", textAlign:"center"}}>CONNECT TO YOUR EXISTING SYSTEMS</div>
            <div style={{display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:"0.75rem", marginBottom:"1.5rem"}}>
              {SOURCES.map((s,i) => (
                <div key={i}
                  ...
                  style={{
                    background: active===i ? T.warm : T.surface,
                    border: active===i ? `1px solid ${T.terra}` : `1px solid ${T.border}`,
                    borderRadius:12, padding:"0.875rem 1rem",
                    display:"flex", alignItems:"center", gap:"0.75rem",
                    cursor:"default", transition:"all .15s",
                    boxShadow: active===i ? `0 4px 12px rgba(196,99,58,0.12)` : T.shadow
                  }}>
                  <span style={{fontSize:"1.4rem"}}>{s.icon}</span>
                  <div>
                    <div style={{fontWeight:700, fontSize:"0.875rem", color:T.text}}>{s.label}</div>
                    <div style={{fontSize:"0.72rem", color:T.muted}}>{s.sub}</div>
                  </div>
                  {active===i && <div style={{marginLeft:"auto", fontSize:"0.7rem", color:T.terra, fontWeight:700}}>✓ Connected</div>}
                </div>
              ))}
            </div>
            {/* Flow diagram */}
            <div style={{background:T.warm, borderRadius:14, padding:"1.25rem", border:`1px solid ${T.border}`, textAlign:"center"}}>
              <div style={{display:"flex", alignItems:"center", justifyContent:"center", gap:"0.5rem", flexWrap:"wrap"}}>
                <div style={{background:T.surface, border:`1px solid ${T.border}`, borderRadius:8, padding:"0.4rem 0.75rem", fontSize:"0.78rem", fontWeight:600, color:T.text}}>Your DBs</div>
                <span style={{color:T.terra, fontWeight:700}}>→</span>
                <div style={{background:T.terra, borderRadius:8, padding:"0.4rem 0.75rem", fontSize:"0.78rem", fontWeight:700, color:"#fff"}}>Supabase Wrappers</div>
                <span style={{color:T.terra, fontWeight:700}}>→</span>
                <div style={{background:T.surface, border:`1px solid ${T.border}`, borderRadius:8, padding:"0.4rem 0.75rem", fontSize:"0.78rem", fontWeight:600, color:T.text}}>Edge Functions</div>
                <span style={{color:T.terra, fontWeight:700}}>→</span>
                <div style={{background:T.surface, border:`1px solid ${T.border}`, borderRadius:8, padding:"0.4rem 0.75rem", fontSize:"0.78rem", fontWeight:600, color:T.text}}>n8n</div>
                <span style={{color:T.terra, fontWeight:700}}>→</span>
                <div style={{background:"#1C1917", borderRadius:8, padding:"0.4rem 0.75rem", fontSize:"0.78rem", fontWeight:600, color:"#fff"}}>Web / WA / TG</div>
              </div>
              <div style={{fontSize:"0.75rem", color:T.muted, marginTop:"0.75rem"}}>One secure pipeline. Zero disruption to your existing systems.</div>
            </div>
          </div>
        </div>
      </div>
